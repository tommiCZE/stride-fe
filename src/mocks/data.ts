...
export const USERS: User[] = [
  { id: 'u1', name: 'Tomáš Veselý',    initials: 'TV', color: '#6366f1', role: 'PM',       email: 'tomas.vesely@acme.cz',    workspaceRole: 'admin',  status: 'active'  },
  { id: 'u2', name: 'Jana Nováková',   initials: 'JN', color: '#ec4899', role: 'Frontend', email: 'jana.novakova@acme.cz',   workspaceRole: 'member', status: 'active'  },
  { id: 'u3', name: 'Pavel Dvořák',    initials: 'PD', color: '#0ea5e9', role: 'Backend',  email: 'pavel.dvorak@acme.cz',    workspaceRole: 'member', status: 'active'  },
  { id: 'u4', name: 'Klára Svobodová', initials: 'KS', color: '#10b981', role: 'Design',   email: 'klara.svobodova@acme.cz', workspaceRole: 'member', status: 'active'  },
  { id: 'u5', name: 'Marek Procházka', initials: 'MP', color: '#f59e0b', role: 'QA',       email: 'marek.prochazka@acme.cz', workspaceRole: 'member', status: 'active'  },
  { id: 'u6', name: 'Eva Horáková',    initials: 'EH', color: '#8b5cf6', role: 'Frontend', email: 'eva.horakova@acme.cz',    workspaceRole: 'viewer', status: 'active'  },
  { id: 'u7', name: 'David Kučera',    initials: 'DK', color: '#ef4444', role: 'DevOps',   email: 'david.kucera@acme.cz',    workspaceRole: 'member', status: 'active'  },
  { id: 'u8', name: 'Lucie Marešová',  initials: 'LM', color: '#14b8a6', role: 'PO',       email: 'lucie.maresova@acme.cz',  workspaceRole: 'member', status: 'pending' },
];
...
export const RICH_DESC_1: RichBlock[] = [
  ...
  { type: 'ul', items: [
    'Heading 1 / 2 / 3', 'Bullet / numbered list', 'Todo list',
    'Quote, Callout, Divider', 'Code block', 'Image (upload + paste)',
    'Table', 'Mention (@user) a Issue link (#TASK-123)',
  ]},
...
export const TASKS: Task[] = [
  {
    ...
    description: RICH_DESC_1,
    updated: '2026-04-27T14:32:00', created: '2026-04-21T09:15:00',
  },
  {
...
export const GIT_INTEGRATIONS: GitIntegration[] = [
  {
...
export const getStatus  = (id: string) => STATUSES.find(s => s.id === id);
export const getPriority = (id: string) => PRIORITIES.find(p => p.id === id);
export const getType    = (id: string) => TASK_TYPES.find(t => t.id === id);
export const getLabel   = (id: string) => LABELS.find(l => l.id === id);
