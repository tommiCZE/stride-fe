...
export default function Column({ status, tasks, onTaskClick }: ColumnProps) {
  ...
  return (
    <Stack
      ...
      sx={{ width: 296, flexShrink: 0, bgcolor: 'action.hover', borderRadius: 1.5, border: 1,
    ...
    >
      <Stack direction="row" spacing={0.75} sx={{ px: 1.25, py: 1, alignItems: 'center' }}>
        ...
        <Typography
          ...
          sx={{ fontSize: '13px', fontWeight: 700, letterSpacing: '0.04em', textTransform: 'uppercase',
          color: 'text.secondary' }}
        >
          {status.name}
        </Typography>
        <Typography aria-hidden="true" sx={{ fontSize: '13px', color: 'text.disabled', fontVariantNumeric: 'tabular-nums' }}>{count}</Typography>
        {status.wip && (
          <Box
            aria-hidden="true"
            sx={{ ml: 0.25, fontSize: '13px', fontWeight: 700, px: 0.5, borderRadius: 0.5,
      ...
      <Stack spacing={0.75}
        ...
        sx={{ px: 1, pb: 1, overflowY: 'auto', flex: 1, minHeight: 0 }}
      >
        <SortableContext items={tasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
          {tasks.map(t => (
            <SortableTaskCard key={t.id} task={t} onOpen={onTaskClick}/>
          ))}
        </SortableContext>
        {tasks.length === 0 && !adding && (
          <Box
            role="status"
            sx={{ p: 2, textAlign: 'center', color: 'text.disabled', fontSize: '13px',
        ...
        {adding ? (
          <TextField
            ...
            sx={{ '& .MuiOutlinedInput-root': { fontSize: '14px', bgcolor: 'background.paper' },
          ...
          <Stack direction="row" spacing={0.5}
            ...
            sx={{ alignItems: 'center', p: 0.5, borderRadius: 1,
              color: 'text.disabled', fontSize: '13px', cursor: 'pointer', userSelect: 'none',
