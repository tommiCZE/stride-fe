...
import type { DevBranch, DevReviewer } from '../../../types/dev-activity';
...
function reviewerRingColor(verdict: DevReviewer['verdict']): string | undefined {
  if (verdict === 'approved')  return 'success.main';
  if (verdict === 'commented') return 'warning.main';
  if (verdict === 'changes')   return 'error.main';
  return undefined;
}

function ReviewerAvatar({ reviewer, size = 18 }: { reviewer: DevReviewer; size?: number }) {
  const ring = reviewerRingColor(reviewer.verdict);
  return (
    <Tooltip title={`${reviewer.user.name} · ${reviewer.verdict}`}>
      <Box sx={theme => ({
        display: 'inline-flex',
        borderRadius: '50%',
        boxShadow: ring ? `0 0 0 1.5px ${theme.palette[ring.split('.')[0] as 'success' | 'warning' | 'error'].main}` : undefined,
      })}>
